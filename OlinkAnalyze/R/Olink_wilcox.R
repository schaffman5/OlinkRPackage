...
#'
#'Performs a Welch 2-sample Mann-Whitney U Test at confidence level 0.95 for every protein (by OlinkID) for a given grouping variable using stats::t.test and corrects for multiple testing by the Benjamini-Hochberg method (“fdr”) using stats::p.adjust.
...
#' @param ... Options to be passed to wilcox.test. See ?wilcox.test for more.
...
olink_wilcox <- function(df, variable, pair_id, ...){
  ...
  if(!missing(pair_id)){
    ...
    p.val <- df %>%
      ...
      do(tidy(wilcox.test(x=.[[var_levels[1]]],y=.[[var_levels[2]]],paired=T, ...))) %>%
      ungroup() %>%
    ...
    p.val <- df %>%
      ...
      filter(!(OlinkID %in% nas_in_level)) %>%
      group_by(Assay, OlinkID, UniProt, Panel) %>%
      do(tidy(wilcox.test(NPX ~ !!rlang::ensym(variable), data = ., ...))) %>%
      ungroup() %>%
      mutate(Adjusted_pval = p.adjust(p.value, method = "fdr")) %>%
      mutate(Threshold = ifelse(Adjusted_pval < 0.05, "Significant", "Non-significant")) %>%
      rename(`:=`(!!var_levels[1], estimate1)) %>%
