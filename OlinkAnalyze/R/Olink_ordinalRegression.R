...
#' anova_results <- olink_ordinalRegression(df = npx_df,
#'                              variable="Treatment:Time")}
#' @import dplyr stringr tidyr car broom ordinal rstatix
...
olink_ordinalRegression<- function(df,
  ...
  withCallingHandlers({
    ...
    p.val <- df %>%
      ...
      mutate(NPX = rank(NPX))%>%
      convert_as_factor(NPX)%>%
      do(tidy(car::Anova(clm(as.formula(formula_string),
                            data=.,
      ...
      ungroup()%>%
      select(-covariates) %>%
      select(Assay,OlinkID,UniProt,Panel,term,df,statistic,p.value,Adjusted_pval,Threshold) %>%
...
olink_ordinalRegression_posthoc <- function(df,
  ...
  withCallingHandlers({
    ...
    anova_posthoc_results <- df %>%
      ...
      do(data.frame(summary(emmeans(clm(as.formula(formula_string),data=.),
      ...
      anova_posthoc_results <- anova_posthoc_results %>%
        ...
        mutate(Threshold = if_else(Adjusted_pval < 0.05,
                                   'Significant',
                                   'Non-significant')) %>%
        select(Assay, OlinkID, UniProt, Panel, term,contrast, estimate, Adjusted_pval,Threshold)
